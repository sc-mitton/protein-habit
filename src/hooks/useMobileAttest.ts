...
import type { AttestError } from "attest";
import {
...
export const useMobileAttest = () => {
  ...
  const handleAttestation = useCallback(async () => {
    ...
    const result = await AttestModule.asyncGenerateKey(challengeResponse);

    // Error check
    if (result && "code" in result && "message" in result) {
      const error = result as AttestError;
