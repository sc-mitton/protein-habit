...
const Appearance = (props: RootScreenProps<"AppearanceModal">) => {
      ...
      >
        {fontOptions.map((font) => (
          <Pressable key={font} onPress={() => dispatch(setFont(font))}>
            <Box
              borderWidth={2}
              borderColor={font === selectedFont ? "selected" : "unselected"}
              padding="s"
              borderRadius="m"
              alignItems="center"
              justifyContent="center"
...
export default function (props: RootScreenProps<"AppearanceModal">) {
  const theme = useTheme();
  return (
    <BottomSheet
